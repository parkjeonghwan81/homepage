...
public class MovieDAO {
	...
	public List<MovieDTO> list(Map map) {
		...
		StringBuffer sql = new StringBuffer();
		sql.append(" SELECT movieno, movname,imgname, title, viewcnt, wdate, content, r  ");
		sql.append(" From( ");
		sql.append(" SELECT movieno, movname,imgname, title, viewcnt, wdate, content,  ");
		...
		try {
			...
			while (rs.next()) {
				...
				dto.setMovname(rs.getString("movname"));
	...
	public boolean create(MovieDTO dto) {
		...
		sql.append(" INSERT INTO movie(movieno, imgname,movname, title, content, passwd, wdate)   ");
		sql.append(" VALUES((SELECT NVL(MAX(movieno), 0) + 1 as movieno FROM movie),  ");
		sql.append(" ?, ?, ?, ?, ?,sysdate )      ");
		...
		try {
			...
			pstmt.setString(2, dto.getMovname());
